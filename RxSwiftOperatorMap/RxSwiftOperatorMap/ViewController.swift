...
extension ViewController {
    /*
     *** MARK: .mapの概念 ***
     .mapはデータストリームによって流れてきた値を、.map内の指定したルールに従い新しいデータを生成し次のストリームに流す役割があります。
     例えると自動翻訳機です。私が日本語を入力したら、システム内で英語に変換するルールがあれば英語でデータが返却されます。またスペイン語で指定したらスペイン語が返ってきます。
     このようにデータの流れがある中で、.mapはその流れの「変換地点」のような存在です。
     */
    func bind() {
        /*
